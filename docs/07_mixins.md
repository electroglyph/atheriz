...
### 7.2.2 Access Is Always Locked
`access` always runs under `with self.lock:` (`atheriz/objects/base_lock.py:32`).

### 7.2.3 `_pickle_excludes`
`access` is excluded via `_pickle_excludes = ("access",)` (`atheriz/objects/base_lock.py:12`) so old pickles that stored an instance-bound `access` (from the removed `_safe_access`/`_fast_access` era) do not restore a stale callable. Current `access` is a normal class method. MRO loop in `atheriz/objects/base_obj.py:401` pops per-class excludes; add custom non-picklable attrs to your class's `_pickle_excludes`.
