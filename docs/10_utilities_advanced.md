...
- `get_reverse_link(location, destination)`: Returns the `NodeLink` in `destination` that leads back to `location` (useful for finding the reverse exit when building doors or symmetric exits).
...
The text displayed to clients before authentication is generated by the `render()` function in `connection_screen.py`. To customize the splash screen, modify the `connection_screen.py` in your game folder and ensure the override is registered in `CLASS_INJECTIONS`.
