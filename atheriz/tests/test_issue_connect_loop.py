...
every choice is out of range, so the loop can never terminate and the player
is stuck at the character prompt forever.
"""
...
class TestConnectNoCharacters:
    def test_connect_with_empty_characters_terminates(
        ...
        """INTENT: connecting an account that has no characters must terminate
        (e.g. with a message), not spin forever waiting for a valid choice."""
